...
* **要做的改造（前端 UI/UX，使用 frontend-design + ui-design）**
  ...
  * 登录页：遇到 DEVICE\_LIMIT\_REACHED 时，不只 toast：弹出“登录冲突”对话框（数据源：`auth.myDevices`），提供：
...
* **要做的改造**
  ...
  * 在关键查询上加超时/慢查询监控（例如 statement\_timeout 或日志采样），为后续优化提供证据。
